...
Key points:
* `TcpListener::bind` binds to an address and returns a listener.
* `listener.accept()` waits for connections, returns a `TcpStream` and address.
* Each connection is handled in a spawned green-thread (`spawn(async move { ... })`).
...
Use `std::env::args()` to choose mode:
